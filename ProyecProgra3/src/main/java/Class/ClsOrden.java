...
public class ClsOrden {
    ...
    String[] nombreclubes = new String[12];//aqui estamos tomando los datos de la clase Matriz
    ...
    public String[][] ordenar(String[][] clubs) {
        ...
        for (int i = 0; i < juegos.length; i++) {//estamos metiendo los resulltados de los equipos de la clase Matriz
            nombreclubes[i] = clubs[i][1];       // dentro de la tabla JTable
        ...
        for (int i = 0; i < (juegos.length - 1); i++) {
            for (int j = 0; j < (juegos.length - 1); j++) {
                ...
                //Ordenar por Puntos en la tabla
                if (puntos[j] < puntos[j + 1]) {
                    pts = puntos[j];
                    nombreequipo = nombreclubes[j];
                    jor = juegos[j];
                    ganar = ganados[j];
                    empatar = empates[j];
                    perder = perdidos[j];
